...
type Message = { id: number; type: 'load'; bytes: ArrayBuffer } | { id: number; type: 'route'; origin: Coordinate; destination: Coordinate; alternatives: number }
...
self.onmessage = async ({ data }: MessageEvent<Message>) => {
  try {
    if (data.type === 'load') { router = await loadWasmRouter(data.bytes); self.postMessage({ id: data.id, ok: true, routes: [] }); return }
...
async function loadWasmRouter(bytes: ArrayBuffer): Promise<WasmRouter> {
  const moduleUrl = new URL('/wasm/ciclybog_router_wasm.js', self.location.origin).href
  ...
  await module.default(new URL('/wasm/ciclybog_router_wasm_bg.wasm', self.location.origin).href)
